...
int wal_init(wal_context_t* wal, const char* storage_path) {
    ...
    if (wal->wal_fd < 0) {
        printf("[WAL] ❌ Failed to open WAL file: %s (errno: %d)\n", 
               wal->wal_path, errno);
        return -1;
    }
    ...
    if (fstat(wal->wal_fd, &st) == 0 && st.st_size > 0) {
        ...
        if (pread(wal->wal_fd, &header, sizeof(header), 0) == sizeof(header)) {
            if (header.magic == WAL_MAGIC) {
                ...
                printf("[WAL] ✅ Loaded existing WAL: sequence=%lu, checkpoint=%lu\n",
                       wal->sequence, wal->checkpoint_sequence);
            } else {
                // Invalid magic - initialize as new
                printf("[WAL] ⚠️  Invalid WAL magic, initializing as new\n");
                wal->sequence = 0;
        ...
        // New WAL file - write header and ensure file is properly initialized
        // Truncate to header size first to clear any garbage data
        if (ftruncate(wal->wal_fd, sizeof(wal_file_header_t)) != 0) {
            printf("[WAL] ❌ Failed to initialize WAL file size\n");
            close(wal->wal_fd);
            return -1;
        }
        ...
        if (pwrite(wal->wal_fd, &header, sizeof(header), 0) != sizeof(header)) {
            printf("[WAL] ❌ Failed to write WAL header\n");
            close(wal->wal_fd);
            return -1;
        }
        fsync(wal->wal_fd);
        ...
        printf("[WAL] ✅ Created new WAL file\n");
    }
    ...
    if (pthread_mutex_init(&wal->flush_mutex, NULL) != 0) {
        printf("[WAL] ❌ Failed to initialize flush mutex\n");
        close(wal->wal_fd);
        return -1;
    }
    
    if (pthread_cond_init(&wal->flush_cond, NULL) != 0) {
        printf("[WAL] ❌ Failed to initialize flush condition variable\n");
...
static int wal_flush_sync(wal_context_t* wal) {
    ...
    if (buffer_pos > 0) {
        ...
        if (flush_buffer) {
            ...
            printf("[WAL] ❌ Failed to allocate flush buffer\n");
    ...
    if (written != (ssize_t)buffer_pos) {
        printf("[WAL] ❌ Failed to flush WAL buffer (written: %zd, expected: %zu)\n", 
               written, buffer_pos);
        return -1;
    }
    ...
    if (fsync(wal->wal_fd) != 0) {
        printf("[WAL] ❌ Failed to fsync\n");
        return -1;
    }
    
    // Update file header with new sequence
    wal_file_header_t header = {
        .magic = WAL_MAGIC,
        .version = 1,
    ...
    while (!wal->flush_thread_stop) {
        ...
        if (buffer_pos > 0) {
            ...
            if (flush_buffer) {
                ...
                printf("[WAL-FLUSH] ❌ Failed to allocate flush buffer\n");
        ...
        if (buffer_pos > 0 && flush_buffer) {
            printf("[WAL-FLUSH] ✅ Flushing batch: %u entries, %zu bytes (%.2f KB)\n", 
            ...
            if (written != (ssize_t)buffer_pos) {
                wal->flush_error = -1;
                printf("[WAL-FLUSH] ❌ Failed to write buffer (written: %zd, expected: %zu)\n", 
                       written, buffer_pos);
            } else {
                ...
                if (fsync(wal->wal_fd) != 0) {
                    wal->flush_error = -1;
                    printf("[WAL-FLUSH] ❌ Failed to fsync\n");
                } else {
                    wal->flush_error = 0;
...
uint64_t wal_append(wal_context_t* wal, wal_operation_t operation,
    ...
    if (!wal || !wal->enabled || wal->wal_fd < 0) return 0;
    ...
    if (wal->batch_size > 0) {
        ...
        if (!wal->write_buffer) {
            ...
            if (!wal->write_buffer) {
                printf("[WAL] ❌ Failed to allocate write buffer\n");
                return 0;
            }
        }
        ...
        if (wal->write_buffer_pos + entry_size > wal->write_buffer_size) {
            ...
            if (!new_buffer) {
                printf("[WAL] ❌ Failed to grow write buffer\n");
                return 0;
            }
    ...
    if (written != sizeof(entry_header)) {
        printf("[WAL] ❌ Failed to write WAL entry header\n");
        return 0;
    }
    ...
    if (data && data_size > 0) {
        ...
        if (written != (ssize_t)data_size) {
            printf("[WAL] ❌ Failed to write WAL entry data\n");
            return 0;
        }
    ...
    (void)pwrite(wal->wal_fd, &header, sizeof(header), 0);
    fsync(wal->wal_fd);
...
int wal_checkpoint(wal_context_t* wal) {
    ...
    if (wal->batch_size > 0 && wal->flush_thread_running) {
        ...
        if (wal_flush_wait(wal, current_sequence) != 0) {
            printf("[WAL] ❌ Flush error during checkpoint\n");
            return -1;
        }
    }
    ...
    if (pwrite(wal->wal_fd, &header, sizeof(header), 0) != sizeof(header)) {
        printf("[WAL] ❌ Failed to update checkpoint in header\n");
        return -1;
    }
    ...
    if (wal_truncate(wal) != 0) {
        printf("[WAL] ⚠️  Failed to truncate WAL after checkpoint\n");
        // Continue anyway - checkpoint is still valid
    }
    
    printf("[WAL] ✅ Checkpointed at sequence %lu\n", wal->checkpoint_sequence);
...
int wal_recover(wal_context_t* wal,
    ...
    struct stat st;
    if (fstat(wal->wal_fd, &st) != 0 || st.st_size == 0) {
        printf("[WAL] ⚠️  WAL file is empty or invalid\n");
    ...
    if (wal_mmap == MAP_FAILED) {
        printf("[WAL] ⚠️  Failed to mmap WAL file, falling back to pread()\n");
        // Fallback to pread() implementation (not shown here for brevity)
        return -1;
    }
    
    // Read file header (direct pointer access - zero copy)
    wal_file_header_t* header = (wal_file_header_t*)wal_mmap;
    
    if (header->magic != WAL_MAGIC) {
        printf("[WAL] ❌ Invalid WAL magic during recovery\n");
    ...
    printf("[WAL] 🔄 Recovering from WAL (mmap'd, checkpoint: %lu, current: %lu)...\n",
           header->checkpoint_sequence, header->sequence);
    
    uint32_t entries_replayed = 0;
    
    // Validate file size is at least header size
    if (file_size < sizeof(wal_file_header_t)) {
        printf("[WAL] ⚠️  WAL file is too small (size: %zu, expected at least %zu)\n",
    ...
    while (offset < file_size) {
        ...
        if (offset + sizeof(wal_entry_header_t) > file_size) {
            printf("[WAL] ⚠️  Incomplete entry header at offset %zu, stopping recovery\n", offset);
            break;
        }
        ...
        if (entry_header->sequence > header->sequence + 1000 || 
            ...
            printf("[WAL] ⚠️  Invalid entry at offset %zu (seq: %lu is out of range, checkpoint: %lu, current: %lu), truncating\n",
            ...
            if (offset <= sizeof(wal_file_header_t) + 1024) {
                printf("[WAL] ⚠️  Corruption detected very early, reinitializing WAL file\n");
                ...
                if (ftruncate(wal->wal_fd, sizeof(wal_file_header_t)) == 0) {
                    ...
                    fsync(wal->wal_fd);
                    printf("[WAL] ✅ Reinitialized WAL file\n");
                }
            } else {
                // Truncate file at this offset to remove corrupted data
                if (ftruncate(wal->wal_fd, offset) == 0) {
                    fsync(wal->wal_fd);
                    printf("[WAL] ✅ Truncated WAL file to remove corrupted data\n");
                }
            }
            break;
        }
        ...
        if (entry_header->data_size > 1048576) {
            ...
            printf("[WAL] ⚠️  Invalid entry at offset %zu (seq: %lu, data_size: %u), truncating\n",
        ...
        if (entry_header->sequence <= header->checkpoint_sequence) {
            ...
            if (entry_header->data_size > 1048576) {
                ...
                printf("[WAL] ⚠️  Found corrupted entry at offset %zu (data_size: %u), truncating WAL file\n",
            ...
            if (offset + entry_header->data_size > file_size) {
                printf("[WAL] ⚠️  Entry data extends beyond file, stopping recovery\n");
                break;
            }
        ...
        if (entry_header->data_size > 0) {
            ...
            // Bounds check
            if (offset + entry_header->data_size > file_size) {
                printf("[WAL] ⚠️  Incomplete entry data at offset %zu (data_size: %u, file_size: %zu)\n", 
        ...
        switch (entry_header->operation) {
            case WAL_OP_ADD_NODE: {
                if (entry_data && entry_header->data_size >= 9) {
                    ...
                    if (data_offset + name_len > entry_header->data_size) {
                        printf("[WAL] ⚠️  Invalid name length in entry\n");
                        break;
                    }
                    ...
                    if (node_data_len > 0) {
                        ...
                        if (data_offset + node_data_len > entry_header->data_size) {
                            printf("[WAL] ⚠️  Invalid node data length in entry\n");
                            free(name);
                            break;
                        }
            ...
            case WAL_OP_CHECKPOINT:
                ...
                break;
            default:
                printf("[WAL] ⚠️  Unknown operation type: %d\n", entry_header->operation);
                break;
        }
        
        if (result != 0) {
            printf("[WAL] ⚠️  Failed to apply operation %d at sequence %lu\n",
    ...
    printf("[WAL] ✅ Recovery complete (mmap'd): %u entries replayed\n", entries_replayed);
...
int wal_truncate(wal_context_t* wal) {
    ...
    if (wal->checkpoint_sequence >= wal->sequence) {
        ...
        if (ftruncate(wal->wal_fd, truncate_offset) != 0) {
            printf("[WAL] ❌ Failed to truncate WAL file\n");
            return -1;
        }
        wal->file_pos = truncate_offset;  // Update file position
        printf("[WAL] ✅ Truncated WAL to %ld bytes (all entries checkpointed)\n", truncate_offset);
        return 0;
    }
    ...
    while (offset < file_size) {
        ...
        if (entry_header.data_size > 1048576) {
            ...
            printf("[WAL] ⚠️  Found corrupted entry at offset %ld, truncating before it\n", offset);
            break;
        }
    ...
    if (truncate_offset > sizeof(wal_file_header_t)) {
        if (ftruncate(wal->wal_fd, truncate_offset) != 0) {
            printf("[WAL] ❌ Failed to truncate WAL file to offset %ld\n", truncate_offset);
            return -1;
        }
        wal->file_pos = truncate_offset;  // Update file position
        ...
        printf("[WAL] ✅ Truncated WAL to %ld bytes (removed checkpointed entries)\n", truncate_offset);
    }
...
int wal_set_batch_size(wal_context_t* wal, uint32_t batch_size) {
    ...
    if (wal->batch_size > 0 && !wal->flush_thread_running) {
        ...
        if (pthread_create(&wal->flush_thread, NULL, wal_flush_thread_func, wal) != 0) {
            printf("[WAL] ❌ Failed to start background flush thread\n");
