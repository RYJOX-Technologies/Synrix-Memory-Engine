...
typedef enum {
    ...
    LATTICE_ERROR_LICENSE_LIMIT = -103     // License node limit reached
} lattice_error_code_t;

// License claims (JWT payload)
typedef struct {
    uint32_t node_limit;          // Required
    uint64_t exp;                 // Required (unix epoch seconds)
    uint64_t iat;                 // Optional
    char tier[32];                // Optional
} lattice_license_claims_t;
...
typedef enum {
    ...
    LATTICE_NODE_INTERFACE = 11,         // Tier-3: machine-verifiable contract (ABI, ownership, etc.)
    LATTICE_NODE_OBJECTIVE = 12,         // Tier-3: vector metrics + policy gates
    LATTICE_NODE_COMPONENT = 13,         // Tier-3: composition of LEARNING_* + interfaces + objective
    LATTICE_NODE_SYSTEM = 14,            // Tier-4: composition of COMPONENT_* + objective
    LATTICE_NODE_TEST = 15,              // Phase 7: test suite (regression, stress); promotion lanes
...
typedef struct {
    ...
    exact_name_index_t* exact_name_index;      // Full name -> node_id (O(1) exact lookups)
    ...
    bool license_active;              // True when a valid license is applied
    uint32_t license_node_limit;      // License node limit (0 = unlimited)
    uint64_t license_exp;             // License expiration (unix epoch seconds)
    char license_tier[32];            // Optional tier label
...
// Apply license claims to a lattice instance
// Returns: 0 on success, -1 on error
int lattice_apply_license(persistent_lattice_t* lattice, const lattice_license_claims_t* claims);

// Set license key at runtime (JWT string). Verification requires app to use lattice_apply_license(claims) after verifying JWT.
// Returns: 0 on success, -1 if key invalid or verification not available in this build.
...
// Only succeeds with verified unlimited (tier-4) key; else returns -1.
int lattice_disable_evaluation_mode(persistent_lattice_t* lattice);

// Global usage tracking: one cap per license per machine.
// Linux: $HOME/.synrix/license_usage/<hash>.dat
// On load: call license_global_register(license_id). Before every add: license_global_add_one(license_id, cap); deny if it returns -1.
