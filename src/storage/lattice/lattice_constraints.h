 ...
 * 
 * @param strict If true, reject nodes with invalid prefixes. If false, warn but allow.
 * 
 * Default: true (strict mode enabled for launch protection)
 */
void lattice_set_prefix_validation_strict(bool strict);

/**
 * Get current prefix validation strictness mode.
 * 
 * @return true if strict mode enabled, false if permissive mode
 */
 ...
 * 
 * In strict mode (default), rejects:
 * - Garbage prefixes (TEMP_, RANDOM_, TEST_, etc.)
 * - Nodes without valid semantic prefixes
 * 
 * In permissive mode, warns but allows invalid prefixes (backward compatibility).
 * 
