...
static bool lattice_limit_exceeded(persistent_lattice_t* lattice,
                                   uint32_t nodes_to_add,
                                   uint32_t* limit_out,
                                   bool* is_license_limit) {
    if (!lattice || nodes_to_add == 0) {
        return false;
    }
    
    if (lattice->license_active && lattice->license_node_limit > 0) {
        uint64_t projected = (uint64_t)lattice->total_nodes + (uint64_t)nodes_to_add;
        if (projected > lattice->license_node_limit) {
            if (limit_out) *limit_out = lattice->license_node_limit;
            if (is_license_limit) *is_license_limit = true;
            return true;
        }
        return false;
    }
    ...
    if (lattice->evaluation_mode && lattice->free_tier_limit > 0) {
        uint64_t projected = (uint64_t)lattice->total_nodes + (uint64_t)nodes_to_add;
        if (projected > lattice->free_tier_limit) {
            if (limit_out) *limit_out = lattice->free_tier_limit;
            if (is_license_limit) *is_license_limit = false;
            return true;
        }
    }
...
// Effective license id and cap for global usage tracking (one cap per license per machine)
static void lattice_effective_license_id_and_cap(const persistent_lattice_t* lattice, char* id_out, size_t id_size, uint32_t* cap_out) {
    if (!id_out || id_size == 0 || !cap_out) return;
    id_out[0] = '\0';
    *cap_out = lattice->free_tier_limit;
    if (!lattice->license_active) {
        strncpy(id_out, "free", id_size - 1);
        id_out[id_size - 1] = '\0';
        return;
    }
    *cap_out = (lattice->license_node_limit > 0) ? lattice->license_node_limit : lattice->free_tier_limit;
    snprintf(id_out, id_size, "tier-%s-%u", lattice->license_tier[0] != '\0' ? lattice->license_tier : "licensed", lattice->license_node_limit);
}
...
int lattice_init(persistent_lattice_t* lattice, const char* storage_path, uint32_t max_nodes, uint32_t device_id) {
    ...
    // For production/creator use, set evaluation_mode=false to disable limits
    lattice->evaluation_mode = true;
    // For testing: SYNRIX_DEV_TIER=1 or SYNRIX_TESTING=1 raises cap so tests don't hit 25k (no DLL swap needed)
    if (getenv("SYNRIX_DEV_TIER") || getenv("SYNRIX_TESTING")) {
        lattice->free_tier_limit = 250000;  // 250k for dev/testing
    } else {
        lattice->free_tier_limit = 25000;   // 25k node limit for free tier (all SDKs/engine default)
    }
    lattice->license_active = false;
    lattice->license_node_limit = 0;
    lattice->license_exp = 0;
    lattice->license_tier[0] = '\0';
    ...
    lattice->exact_name_index = exact_name_index_create();
    ...
    if (lattice_enable_wal(lattice) != 0) {
        printf("[LATTICE] ⚠️  Failed to enable WAL (continuing without crash recovery)\n");
    }
...
int lattice_init_disk_mode(persistent_lattice_t* lattice, const char* storage_path, 
    ...
    if (file_exists && existing_size == file_size) {
        ...
        printf("[LATTICE-INIT-DISK] ✅ Using existing file (%.2f GB)\n", file_size / (1024.0 * 1024.0 * 1024.0));
    } else {
        ...
        printf("[LATTICE-INIT-DISK] ✅ File pre-allocated (uninitialized slots will be skipped during indexing)\n");
        fflush(stdout);
    }
    ...
    printf("[LATTICE-INIT-DISK] ✅ Memory-mapped %.2f GB with MAP_SHARED\n", file_size / (1024.0 * 1024.0 * 1024.0));
    fflush(stdout);
    ...
    printf("[LATTICE-INIT-DISK] ✅ Metadata arrays allocated\n");
    fflush(stdout);
    ...
    // Enable WAL by default for crash recovery (production requirement)
    if (lattice_enable_wal(lattice) != 0) {
        printf("[LATTICE-INIT-DISK] ⚠️  Failed to enable WAL (continuing without crash recovery)\n");
    }
    ...
    if (file_exists && existing_size == file_size) {
        ...
        if (read_bytes == sizeof(header)) {
            if (header[0] == 0x4C415454) {
                ...
                printf("[LATTICE-INIT-DISK] ✅ Found existing file: %u nodes, next_id=%lu\n",
            ...
            for (uint32_t i = 0; i < nodes_to_index; i++) {
                ...
                printf("[LATTICE-INIT-DISK] ✅ Indexed %u nodes in RAM cache (of %u total in file)\n",
                       lattice->node_count, lattice->total_nodes);
            } else {
                // Invalid magic - treat as new
                printf("[LATTICE-INIT-DISK] ⚠️  File exists but invalid magic (0x%08X), initializing as new\n", header[0]);
                lattice->node_count = 0;
                lattice->next_id = 1;
                lattice->total_nodes = 0;
            ...
            printf("[LATTICE-INIT-DISK] ⚠️  Failed to read header (read %zd bytes), initializing as new\n", read_bytes);
            lattice->node_count = 0;
            lattice->next_id = 1;
            lattice->total_nodes = 0;
        }
    ...
    printf("[LATTICE-INIT-DISK] ✅ Disk mode initialized (kernel will manage dirty page flushing)\n");
    fflush(stdout);
...
void lattice_cleanup(persistent_lattice_t* lattice) {
    ...
    if (lattice->exact_name_index) {
        exact_name_index_destroy(lattice->exact_name_index);
        lattice->exact_name_index = NULL;
    }
...
int lattice_save(persistent_lattice_t* lattice) {
    ...
    if (lattice->disk_mode) {
        ...
        // Sync only the written portion of the mmap'd region (not the entire file)
        // This is much faster and only syncs dirty pages that have actually been written
        // CRITICAL: Check mmap_ptr is still valid (not yet unmapped)
        if (lattice->mmap_ptr && lattice->mmap_ptr != MAP_FAILED && 
    ...
    for (uint32_t i = 0; i < lattice->node_count; i++) {
        ...
        if (write(fd, node, sizeof(lattice_node_t)) != sizeof(lattice_node_t)) {
            close(fd);
            unlink(temp_path);
            return -1;
        }
...
int lattice_load(persistent_lattice_t* lattice) {
    ...
    if (fd < 0) {
        ...
        printf("[LATTICE-LOAD] ❌ Failed to open file: %s (errno=%d)\n", lattice->storage_path, errno);
        return -1;
    }
    printf("[LATTICE-LOAD] ✅ Opened file: %s\n", lattice->storage_path);
    ...
    // Verify magic
    if (header[0] != 0x4C415454) {
        printf("[LATTICE-LOAD] ❌ Invalid magic: expected 0x4C415454, got 0x%08X\n", header[0]);
        close(fd);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < nodes_to_read; i++) {
                ...
                if (strncmp(node.name, "C:", 2) != 0 && 
                    ...
                    if (looks_like_data) {
                        ...
                        if (corrupted_nodes_skipped <= 3) {
                            printf("[LATTICE-LOAD] ⚠️  CORRUPTION: Invalid node ID %lu (local_id=%u) at position %u - SKIPPING\n",
                                   node.id, local_id, i);
                        } else if (corrupted_nodes_skipped == 4) {
                            printf("[LATTICE-LOAD] ⚠️  (Additional corrupted nodes will be skipped silently...)\n");
                        }
                    }
                    is_corrupted = true;
                }
        ...
        if (!is_corrupted && !is_valid_type && !is_chunked_by_name && !is_chunked_by_type) {
            ...
            if (corrupted_nodes_skipped <= 3) {
                printf("[LATTICE-LOAD] ⚠️  CORRUPTION: Invalid node type %d for node %lu - SKIPPING\n",
                       node.type, node.id);
            } else if (corrupted_nodes_skipped == 4) {
                printf("[LATTICE-LOAD] ⚠️  (Additional corrupted nodes will be skipped silently...)\n");
            }
        ...
        if (!is_corrupted && node.child_count > 1000) {
            ...
            corruption_already_counted = true;
            if (corrupted_nodes_skipped <= 3) {
                printf("[LATTICE-LOAD] ⚠️  CORRUPTION: Invalid child_count %u for node %lu - SKIPPING\n",
        ...
        if (lattice->id_to_index_map) {
            ...
            if (local_id >= estimated_current_size) {
                ...
                if (new_id_to_index) {
                    ...
                    printf("[LATTICE-LOAD] ❌ Failed to grow id_to_index_map to %u\n", new_index_size);
                }
            }
            if (local_id < estimated_current_size) {
                lattice->id_to_index_map[local_id] = lattice->node_count;
    ...
    if (corrupted_nodes_skipped > 0) {
        printf("[LATTICE-LOAD] ⚠️  Loaded %u nodes, skipped %u corrupted nodes\n",
               lattice->node_count, corrupted_nodes_skipped);
        printf("[LATTICE-LOAD] 💡 Run lattice_compact_file() to clean up the file\n");
    } else {
        printf("[LATTICE-LOAD] ✅ Loaded %u nodes (no corruption detected)\n", lattice->node_count);
    }
...
// Add node to lattice with intelligent memory management
uint64_t lattice_add_node(persistent_lattice_t* lattice, lattice_node_type_t type, 
    ...
    // Clear any previous error at start
    lattice->last_error = LATTICE_ERROR_NONE;
    
    // GLOBAL USAGE: one cap per license per machine (Linux: $HOME/.synrix/license_usage/<hash>.dat)
    char license_id[64];
    uint32_t cap = 0;
    lattice_effective_license_id_and_cap(lattice, license_id, sizeof(license_id), &cap);
    if (license_global_register(license_id) == 0 && license_global_add_one(license_id, cap) != 0) {
        lattice->last_error = LATTICE_ERROR_FREE_TIER_LIMIT;
        fprintf(stderr,
                "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                "  SYNRIX: Global usage limit reached (this machine)\n"
                "  Limit: %u nodes. Upgrade or free space.\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n", cap);
        fflush(stderr);
        return 0;
    }
    
    // LICENSE / FREE TIER LIMIT ENFORCEMENT
    // Check BEFORE memory allocation to prevent OOM on small devices
    uint32_t limit = 0;
    bool is_license_limit = false;
    if (lattice_limit_exceeded(lattice, 1, &limit, &is_license_limit)) {
        if (is_license_limit) {
            lattice->last_error = LATTICE_ERROR_LICENSE_LIMIT;
            fprintf(stderr,
                    "\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    "  SYNRIX_LICENSE_LIMIT_REACHED\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                    "  Current: %u\n"
                    "  Limit: %u\n"
                    "  Upgrade: https://synrix.ai/upgrade\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n",
                    lattice->total_nodes, limit);
        } else {
            lattice->last_error = LATTICE_ERROR_FREE_TIER_LIMIT;
            fprintf(stderr, 
                    "\n"
    ...
    if (!lattice_validate_prefix_semantics(name)) {
        // In strict mode (default), validation rejects invalid prefixes
        // This prevents prefix explosion and maintains O(k) query performance
        lattice->last_error = LATTICE_ERROR_INVALID_NODE;
        return 0;  // Reject node with invalid prefix
    }
    ...
    if (data) {
        ...
        if (data_len > sizeof(((lattice_node_t*)0)->data) - 1) {
            fprintf(stderr, 
                    "[LATTICE-CONSTRAINTS] ❌ VIOLATION: Data length (%zu) exceeds fixed-size node capacity (%zu)\n"
    ...
    if (!lattice_validate_single_writer()) {
        fprintf(stderr, 
                "[LATTICE-CONSTRAINTS] ❌ VIOLATION: Multiple writers detected\n"
    ...
    if (lattice->disk_mode) {
        if (lattice->total_nodes >= lattice->total_file_nodes) {
            printf("[LATTICE] ❌ Disk mode: Cannot add node, file is full (%u/%u nodes)\n",
    ...
    if (!lattice->disk_mode && lattice->node_count >= lattice->max_nodes) {
        printf("[LATTICE] 🔄 Growing lattice from %u to %u nodes (intelligent memory management)\n", 
        ...
        if (!new_node_id_map) {
            printf("[LATTICE] ❌ Failed to grow node ID map - out of memory\n");
            free(new_nodes);
            return 0;
        }
        ...
        if (!new_access_count) {
            printf("[LATTICE] ❌ Failed to grow access_count - out of memory\n");
        ...
        if (!new_last_access) {
            printf("[LATTICE] ❌ Failed to grow last_access - out of memory\n");
        ...
        if (new_index_size > current_index_size && new_index_size <= max_safe_index_size) {
            uint32_t* new_id_to_index = (uint32_t*)realloc(lattice->id_to_index_map, new_index_size * sizeof(uint32_t));
            if (!new_id_to_index) {
                printf("[LATTICE] ❌ Failed to grow id_to_index_map - out of memory\n");
        ...
        printf("[LATTICE] ✅ Lattice grown to %u nodes (%.1f MB RAM, fully dynamic)\n",
    ...
    if (!lattice->disk_mode && write_index >= lattice->max_nodes) {
        ...
        if (write_index >= lattice->max_nodes) {
            ...
            lattice_node_t* new_nodes = (lattice_node_t*)realloc(lattice->nodes, new_max * sizeof(lattice_node_t));
            if (!new_nodes) {
                printf("[LATTICE] ❌ Failed to grow nodes array - out of memory\n");
                return 0;
            }
            lattice->nodes = new_nodes;
            lattice->max_nodes = new_max;
            printf("[LATTICE] ✅ Emergency grow: expanded to %u nodes\n", new_max);
        }
    }
    
    // Bounds check for disk mode
    if (lattice->disk_mode && write_index >= lattice->total_file_nodes) {
        printf("[LATTICE] ❌ Disk mode: write_index %u >= total_file_nodes %u\n",
    ...
    if (lattice->persistence.auto_save_enabled) {
        ...
        if (should_save_nodes || should_save_time || should_save_pressure) {
            ...
            if (lattice_save(lattice) == 0) {
                ...
                printf("[LATTICE-AUTO-SAVE] ✅ Snapshot saved and checkpointed\n");
                lattice->persistence.nodes_since_last_save = 0; // Reset counter after save
                lattice->persistence.last_save_timestamp = get_current_timestamp();
                fflush(stdout);
            } else {
                printf("[LATTICE-AUTO-SAVE] ⚠️  Failed to save snapshot (will retry on next interval)\n");
                fflush(stdout);
            }
        }
    }
...
uint64_t lattice_add_node_with_id(persistent_lattice_t* lattice, uint32_t reserved_local_id,
    ...
    if (lattice->node_count >= lattice->max_nodes) {
        printf("[LATTICE] 🔄 Growing lattice from %u to %u nodes (thread-safe mode)\n", 
               lattice->max_nodes, lattice->max_nodes * 2);
...
uint64_t lattice_add_node_compressed(persistent_lattice_t* lattice, lattice_node_type_t type,
    ...
    if (compressed_data_len > max_data_size) {
        fprintf(stderr, 
                "[LATTICE-CONSTRAINTS] ❌ VIOLATION: Compressed data length (%zu) exceeds fixed-size node capacity (%zu)\n"
                "This breaks the Lattice. Use chunked storage for large data.\n",
    ...
    if (lattice->wal_enabled && lattice->wal) {
        ...
        if (name_len > UINT32_MAX || compressed_data_len > UINT32_MAX) {
            printf("[LATTICE] ⚠️  WARNING: name_len (%zu) or compressed_data_len (%zu) exceeds uint32_t max, skipping WAL entry\n", name_len, compressed_data_len);
        } else {
            ...
            if (total_size > UINT32_MAX) {
                printf("[LATTICE] ⚠️  WARNING: total_size (%zu) exceeds uint32_t max, skipping WAL entry\n", total_size);
            } else {
...
uint64_t lattice_add_node_binary(persistent_lattice_t* lattice, lattice_node_type_t type,
    ...
    if (data_len > max_binary_size) {
        fprintf(stderr, 
                "[LATTICE-CONSTRAINTS] ❌ VIOLATION: Binary data length (%zu) exceeds fixed-size node capacity (%zu)\n"
                "This breaks the Lattice. Use chunked storage for large data.\n",
    ...
    if (name) {
        ...
        if (strncmp(name, "CHUNK:", 6) == 0 && strncmp(node->name, "CHUNK:", 6) != 0) {
            printf("[LATTICE-BINARY] ❌ Name corruption detected! Input: '%s', Stored: '%s'\n", name, node->name);
        }
    } else {
        node->name[0] = '\0';
    }
    ...
    if (lattice->wal_enabled && lattice->wal) {
        ...
        if (name_len > UINT32_MAX || data_len > UINT32_MAX) {
            printf("[LATTICE] ⚠️  WARNING: name_len (%zu) or data_len (%zu) exceeds uint32_t max, skipping WAL entry\n", name_len, data_len);
        } else {
            uint32_t name_len_32 = (uint32_t)name_len;
            ...
            if (packed_size > UINT32_MAX) {
                printf("[LATTICE] ⚠️  WARNING: packed_size (%zu) exceeds uint32_t max, skipping WAL entry\n", packed_size);
            } else {
                ...
                if (packed_data) {
                    ...
                    wal_append(lattice->wal, WAL_OP_ADD_NODE, node->id, packed_data, packed_size_32);
                    free(packed_data);
                }
            }
        }
...
// ⚠️ DEPRECATED: Returns pointer that becomes invalid if lattice_add_node() triggers realloc
...
lattice_node_t* lattice_get_node(persistent_lattice_t* lattice, uint64_t id) {
    ...
    if (lattice->id_to_index_map && local_id < lattice->max_nodes * 10) {
        ...
        if (index < lattice->node_count && lattice->node_id_map[index] == id) {
            ...
            return &lattice->nodes[index];  // ⚠️ UNSAFE: Pointer invalidated by realloc
        }
    }
    ...
    for (uint32_t i = 0; i < lattice->node_count; i++) {
        if (lattice->node_id_map[i] == id) {
            ...
            return &lattice->nodes[i];  // ⚠️ UNSAFE: Pointer invalidated by realloc
        }
    }
                            ...
                            if (read(fd, &node, sizeof(lattice_node_t)) == sizeof(lattice_node_t)) {
                                ...
                                if (node.id == id || node_local_id == local_id) {
                                    ...
                                    if (lattice->node_count < lattice->max_nodes) {
                                        ...
                                        close(fd);
                                        return &lattice->nodes[idx]; // ⚠️ UNSAFE: Pointer invalidated by realloc
                                    }
                                    close(fd);
...
int lattice_validate_data_access(persistent_lattice_t* lattice, uint64_t id, bool expecting_text) {
    ...
    if (expecting_text && is_binary) {
        fprintf(stderr, "[LATTICE] ⚠️  WARNING: Node %lu contains BINARY data, but text API was used.\n", id);
...
int lattice_update_node_binary(persistent_lattice_t* lattice, uint64_t id, const void* data, size_t data_len) {
    ...
    if (data_len > sizeof(node->data)) {
        printf("[LATTICE] ❌ Binary data too large: %zu bytes (max %zu)\n", 
    ...
    if (lattice->wal_enabled && lattice->wal) {
        ...
        if (data_len > UINT32_MAX - 2) {
            printf("[LATTICE] ⚠️  WARNING: data_len (%zu) exceeds uint32_t max - 2, skipping WAL entry\n", data_len);
        } else {
...
int lattice_store_embedding_metadata(persistent_lattice_t* lattice, uint64_t node_id,
    ...
    if (metadata_size > 510) {
        printf("[LATTICE-EMBEDDING] ❌ Metadata too large (%zu bytes, max 510)\n", metadata_size);
        return -1;
    }
...
uint64_t lattice_add_node_chunked(persistent_lattice_t* lattice,
    ...
    if (chunk_count == 0) {
        printf("[LATTICE-CHUNK] ❌ Invalid chunk count\n");
        return 0;
    }
    ...
    uint32_t limit = 0;
    bool is_license_limit = false;
    if (lattice_limit_exceeded(lattice, total_nodes_needed, &limit, &is_license_limit)) {
        if (is_license_limit) {
            lattice->last_error = LATTICE_ERROR_LICENSE_LIMIT;
            fprintf(stderr,
                    "\n"
                    ...
                    "  Upgrade: https://synrix.ai/upgrade\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n",
                    lattice->total_nodes, limit, total_nodes_needed, chunk_count);
        } else {
            lattice->last_error = LATTICE_ERROR_FREE_TIER_LIMIT;
            fprintf(stderr, 
                    "\n"
                    ...
                    "  • Contact support for assistance\n"
                    "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n",
                    limit, lattice->total_nodes, total_nodes_needed, chunk_count);
        }
        return 0;
    }
    ...
    if (!chunk_ids) {
        printf("[LATTICE-CHUNK] ❌ Failed to allocate chunk IDs array\n");
        return 0;
    }
    ...
    for (uint32_t i = 0; i < chunk_count; i++) {
        ...
        if (name_len < 0 || name_len >= (int)sizeof(chunk_name)) {
            printf("[LATTICE-CHUNK] ❌ Failed to create chunk name (snprintf returned %d)\n", name_len);
            free(chunk_ids);
            free(metadata);
            return 0;
        }
        ...
        if (strncmp(chunk_name, "C:", 2) != 0) {
            printf("[LATTICE-CHUNK] ❌ Chunk name missing 'C:' prefix: '%s'\n", chunk_name);
            free(chunk_ids);
            free(metadata);
            return 0;
        }
    ...
    if (update_result != 0) {
        printf("[LATTICE-CHUNK] ⚠️  Warning: Parent metadata update returned %d\n", update_result);
    }
    ...
    // Uncomment for debugging: printf("[LATTICE-CHUNK] ✅ Created chunked data: %zu bytes in %u chunks (parent: %lu)\n", data_len, chunk_count, parent_node_id);
...
int lattice_get_node_chunked(persistent_lattice_t* lattice,
    ...
    if (lattice_get_node_data(lattice, parent_id, &parent) != 0) {
        printf("[LATTICE-CHUNK] ❌ Parent node %lu not found\n", parent_id);
        return -1;
    }
    
    // Verify it's a chunked node
        if (strncmp(parent.name, "C:", 2) != 0) {
        printf("[LATTICE-CHUNK] ❌ Node %lu is not a chunked data header\n", parent_id);
        return -1;
    }
    ...
    if (chunk_count == 0 || total_size == 0) {
        printf("[LATTICE-CHUNK] ❌ Invalid metadata: size=%zu, chunks=%u\n", 
    ...
    if (!reassembled) {
        printf("[LATTICE-CHUNK] ❌ Failed to allocate %zu bytes\n", total_size);
        return -1;
    }
    ...
    if (!use_sequential_path) {
        ...
        if (!candidate_ids && lattice->disk_mode) {
            ...
            printf("[LATTICE-CHUNK] ⚠️  Fallback: Scanning file indices %u-%u for chunks...\n",
                   scan_start, scan_end);
        ...
        if (candidate_count < chunk_count) {
            printf("[LATTICE-CHUNK] ⚠️  Found %u chunks, expected %u\n", 
                   candidate_count, chunk_count);
    ...
    for (uint32_t i = 0; i < chunk_count && remaining > 0; i++) {
        ...
        if (lattice_get_node_data(lattice, chunk_ids[i], &chunk) != 0) {
            printf("[LATTICE-CHUNK] ⚠️  Chunk %lu not found, skipping\n", chunk_ids[i]);
            continue;
        }
        ...
        if (lattice_get_node_data_binary(lattice, chunk_ids[i], chunk_data, 
            ...
            if (chunk_data_len >= 10) {
                ...
                if (chunk_index != i) {
                    printf("[LATTICE-CHUNK] ⚠️  Chunk index mismatch: expected %u, got %lu\n",
                           i, chunk_index);
                }
    ...
    *out_data_len = total_size - remaining;
    ...
    printf("[LATTICE-CHUNK] ✅ Reassembled %zu bytes from %u chunks\n", 
...
int lattice_add_child_with_metadata(persistent_lattice_t* lattice, 
    ...
    if (metadata_size > 510) {
        printf("[LATTICE-EDGE] ❌ Edge metadata too large (%zu bytes, max 510)\n", metadata_size);
        return -1;
    }
...
void lattice_build_prefix_index(persistent_lattice_t* lattice) {
    ...
    if (lattice->prefix_index.use_dynamic_index) {
        ...
        if (node_names && node_ids) {
            ...
            if (lattice->exact_name_index)
                exact_name_index_build(lattice->exact_name_index, node_names, node_ids, valid_count);
            }
            free(node_names);
            free(node_ids);
        }
    ...
    /* Build exact-name index from all nodes (same as dynamic path: full scan) */
    if (lattice->exact_name_index) {
        for (uint32_t i = 0; i < lattice->node_count; i++) {
            if (lattice->nodes[i].id != 0)
                exact_name_index_put(lattice->exact_name_index, lattice->nodes[i].name, lattice->nodes[i].id);
        }
        if (lattice->mmap_ptr && lattice->mmap_ptr != MAP_FAILED && nodes_to_index > lattice->node_count) {
            size_t header_size = 4 * sizeof(uint32_t);
            for (uint32_t i = lattice->node_count; i < nodes_to_index; i++) {
                ...
                if (node_offset + sizeof(lattice_node_t) <= lattice->mmap_size) {
                    lattice_node_t* disk_node = (lattice_node_t*)((char*)lattice->mmap_ptr + node_offset);
                    if (disk_node->id != 0)
                        exact_name_index_put(lattice->exact_name_index, disk_node->name, disk_node->id);
                }
            }
        }
    }
...
bool lattice_validate_prefix_indexes(persistent_lattice_t* lattice) {
    ...
    for (size_t i = 0; i < sizeof(known_prefixes) / sizeof(known_prefixes[0]); i++) {
        ...
        if (strcmp(prefix, "ISA_") == 0) {
            ...
            if (hardcoded_count != combined_dynamic_count) {
                printf("[VALIDATE] ❌ Mismatch for prefix 'LEARNING_': hardcoded=%u (includes PATTERN_), dynamic=%u (LEARNING_=%u + PATTERN_=%u)\n",
                       hardcoded_count, combined_dynamic_count, learning_dynamic_count, pattern_count);
                all_match = false;
                discrepancies++;
            } else {
                printf("[VALIDATE] ✅ Prefix 'LEARNING_': %u nodes match (hardcoded includes PATTERN_, dynamic separates)\n", 
                       hardcoded_count);
            }
        ...
        if (strcmp(prefix, "PATTERN_") == 0) {
            ...
            printf("[VALIDATE] ℹ️  Prefix 'PATTERN_': hardcoded groups with LEARNING_ (%u total), dynamic separates (%u PATTERN_ nodes)\n",
        ...
        if (hardcoded_count != dynamic_count) {
            printf("[VALIDATE] ❌ Mismatch for prefix '%s': hardcoded=%u, dynamic=%u\n",
        ...
        if (hardcoded_count > 0 && hardcoded_ids && dynamic_ids) {
            ...
            for (uint32_t j = 0; j < hardcoded_count; j++) {
                ...
                if (!found) {
                    printf("[VALIDATE] ❌ ID %lu in hardcoded '%s' not found in dynamic\n",
        ...
        if (hardcoded_count == dynamic_count && hardcoded_count > 0) {
            printf("[VALIDATE] ✅ Prefix '%s': %u nodes match\n", prefix, hardcoded_count);
        }
    }
    ...
    for (uint32_t i = 0; i < lattice->prefix_index.dynamic_index.entry_count; i++) {
        ...
        if (!is_known) {
            printf("[VALIDATE] ℹ️  Dynamic index found new prefix '%s' with %u nodes (not in hardcoded)\n",
                   prefix, lattice->prefix_index.dynamic_index.entries[i].count);
        }
    }
    
    if (all_match && discrepancies == 0) {
        printf("[VALIDATE] ✅ All prefix indexes match perfectly!\n");
    } else {
        printf("[VALIDATE] ⚠️  Found %u discrepancies\n", discrepancies);
    }
...
void lattice_prefix_index_add_node(persistent_lattice_t* lattice, uint64_t node_id, const char* node_name) {
    ...
    if (lattice->exact_name_index && node_id != 0)
        exact_name_index_put(lattice->exact_name_index, node_name, node_id);
...
uint32_t lattice_find_nodes_by_name_filtered(persistent_lattice_t* lattice, const char* name,
    ...
    // Exact-name fast path: O(1) lookup so we never touch the prefix-bucket scan
    if (lattice->exact_name_index && max_ids > 0) {
        uint64_t exact_id = exact_name_index_get(lattice->exact_name_index, name);
        if (exact_id != 0) {
            if (!has_filters) {
                node_ids[0] = exact_id;
                return 1;
            ...
            lattice_node_t node;
            if (lattice_get_node_data(lattice, exact_id, &node) == 0) {
                if ((min_confidence <= 0.0 || node.confidence >= min_confidence) &&
                    (min_timestamp == 0 || node.timestamp >= min_timestamp) &&
                    (max_timestamp == 0 || node.timestamp <= max_timestamp)) {
                    node_ids[0] = exact_id;
...
// Performance: <1μs (direct memory access, no I/O)
        ...
        if (node_id < lattice->total_nodes) {
            ...
            if (node_offset + sizeof(lattice_node_t) <= lattice->mmap_size) {
                ...
                if (mmap_node->id == node_id) {
                    ...
                    return mmap_node; // ⚠️ UNSAFE: Pointer from mmap, but stable
                }
            }
        }
    }
...
int lattice_add_node_streaming(persistent_lattice_t* lattice, lattice_node_type_t type, 
    ...
    // NOTE: Removed blocking msync(MS_SYNC) from hot path to maintain ~28μs write latency.
    ...
    // - Blocking msync would cause ~28ms latency (1000× slower)
...
int lattice_scan_and_repair_corruption(persistent_lattice_t* lattice) {
    ...
    if (corrupted_count > 0) {
        printf("[LATTICE-REPAIR] ⚠️  Detected %u corrupted nodes, marked %u for removal\n",
               corrupted_count, repaired_count);
        // Suggest compaction
        printf("[LATTICE-REPAIR] 💡 Run lattice_compact_file() to remove corrupted nodes\n");
...
int lattice_compact_file(persistent_lattice_t* lattice) {
    ...
    printf("[LATTICE-COMPACT] 🔄 Starting file compaction...\n");
    ...
    if (!valid_nodes || !valid_node_ids) {
        printf("[LATTICE-COMPACT] ❌ Failed to allocate memory\n");
    ...
    printf("[LATTICE-COMPACT] ✅ Found %u valid nodes, removing %u corrupted/uninitialized nodes\n",
           valid_count, removed_count);
    
    if (valid_count == 0) {
        printf("[LATTICE-COMPACT] ⚠️  No valid nodes found, cannot compact\n");
        free(valid_nodes);
        free(valid_node_ids);
        return -1;
    }
    ...
    // Save compacted file
    if (lattice_save(lattice) != 0) {
        printf("[LATTICE-COMPACT] ❌ Failed to save compacted file\n");
        return -1;
    }
    
    printf("[LATTICE-COMPACT] ✅ Compaction complete: %u nodes remaining (removed %u)\n",
           valid_count, removed_count);
...
void lattice_configure_persistence(persistent_lattice_t* lattice,
    ...
    printf("[LATTICE-PERSISTENCE] ✅ Configured: auto_save=%s, interval_nodes=%u, interval_seconds=%u, save_on_pressure=%s\n",
...
int lattice_enable_wal(persistent_lattice_t* lattice) {
    ...
    if (!lattice->wal) {
        printf("[LATTICE-WAL] ❌ Failed to allocate WAL context\n");
        return -1;
    }
    
    // Initialize WAL
    if (wal_init(lattice->wal, lattice->storage_path) != 0) {
        printf("[LATTICE-WAL] ❌ Failed to initialize WAL\n");
        free(lattice->wal);
        lattice->wal = NULL;
        return -1;
    }
    ...
    // For free tier (100k nodes): batch size = 50k (half of limit, ensures checkpoint before limit)
    ...
    printf("[LATTICE-WAL] ✅ WAL enabled for lattice (batching: %u entries, tier-scaled)\n", optimal_batch_size);
...
void lattice_disable_wal(persistent_lattice_t* lattice) {
    ...
    printf("[LATTICE-WAL] ✅ WAL disabled\n");
}
...
int lattice_wal_checkpoint(persistent_lattice_t* lattice) {
    ...
    printf("[LATTICE-WAL] 🔄 Applying WAL entries to main file before checkpoint...\n");
    if (lattice_recover_from_wal(lattice) != 0) {
        printf("[LATTICE-WAL] ⚠️  Failed to apply WAL entries (continuing anyway)\n");
    }
    
    // Save lattice (write applied nodes to file)
    if (lattice_save(lattice) != 0) {
        printf("[LATTICE-WAL] ⚠️  Failed to save lattice before checkpoint\n");
        return -1;
    }
    
    // Checkpoint WAL (mark entries as applied)
    if (wal_checkpoint(lattice->wal) != 0) {
        printf("[LATTICE-WAL] ❌ Failed to checkpoint WAL\n");
        return -1;
    }
    
    printf("[LATTICE-WAL] ✅ WAL entries applied and checkpointed\n");
    return 0;
}
...
int lattice_recover_from_wal(persistent_lattice_t* lattice) {
    ...
    int apply_add_node_cb(void* ctx, uint64_t node_id, uint8_t type,
        ...
        // Uncompressed data - use normal API
        bool wal_was_enabled = (l->wal && l->wal_enabled);
        if (l->wal) {
            l->wal_enabled = false;  // Disable WAL during recovery
        }
        
        uint64_t added_id = lattice_add_node(l, (lattice_node_type_t)type, name, data, parent_id);
    ...
    // Recover from WAL
    printf("[LATTICE-WAL] 🔄 Recovering from WAL...\n");
    if (wal_recover(lattice->wal, apply_add_node_cb, apply_update_node_cb,
                   apply_delete_node_cb, apply_add_child_cb, lattice) != 0) {
        printf("[LATTICE-WAL] ❌ Failed to recover from WAL\n");
        return -1;
    }
    
    printf("[LATTICE-WAL] ✅ Recovery complete\n");
...
int lattice_enable_isolation(persistent_lattice_t* lattice) {
    ...
    if (!lattice->isolation) {
        printf("[LATTICE-ISOLATION] ❌ Failed to allocate isolation context\n");
        return -1;
    }
    
    // Initialize isolation
    if (isolation_init(lattice->isolation) != 0) {
        printf("[LATTICE-ISOLATION] ❌ Failed to initialize isolation\n");
        free(lattice->isolation);
        lattice->isolation = NULL;
        return -1;
    }
    ...
    printf("[LATTICE-ISOLATION] ✅ Isolation enabled for lattice\n");
...
void lattice_disable_isolation(persistent_lattice_t* lattice) {
    ...
    printf("[LATTICE-ISOLATION] ✅ Isolation disabled\n");
}
...
int lattice_apply_license(persistent_lattice_t* lattice, const lattice_license_claims_t* claims) {
    if (!lattice || !claims) return -1;
    /* node_limit 0 = unlimited (tier 4); otherwise must be > 0 */
    if (claims->node_limit != 0 && claims->node_limit < 25000u) return -1;

    lattice->license_active = true;
    lattice->license_node_limit = claims->node_limit;  /* 0 = unlimited */
    lattice->license_exp = claims->exp;
    lattice->evaluation_mode = false;
    lattice->free_tier_limit = 0;
    
    if (claims->tier[0] != '\0') {
        strncpy(lattice->license_tier, claims->tier, sizeof(lattice->license_tier) - 1);
        lattice->license_tier[sizeof(lattice->license_tier) - 1] = '\0';
    } else {
        lattice->license_tier[0] = '\0';
    }
...
// Set license key at runtime. Stub: verification is done by the application; use lattice_apply_license(claims) after verifying JWT.
int lattice_set_license_key(persistent_lattice_t* lattice, const char* key) {
    (void)lattice;
    (void)key;
    return -1;  // Application must verify JWT and call lattice_apply_license()
}

// Disable evaluation mode (unlimited nodes). Only allowed with verified unlimited (tier-4) key; else returns -1.
int lattice_disable_evaluation_mode(persistent_lattice_t* lattice) {
    if (!lattice) return -1;
    if (!lattice->license_active) return -1;
    if (lattice->license_node_limit != 0) return -1;
    // Require tier "unlimited" or "tier-4" (case-insensitive)
    const char* t = lattice->license_tier;
    int ul = (strcasecmp(t, "unlimited") == 0) || (strcasecmp(t, "tier-4") == 0);
    if (!ul) return -1;
    lattice->evaluation_mode = false;
