...
[CRASH-TEST] ✅ File exists: /tmp/aion_crash_tests/crash_test.lattice (size=608016 bytes)
...
```bash
# Build crash_test (requires lattice sources)
gcc -O2 -std=c11 -I. -I./src/storage/lattice \
    tools/crash_test.c \
    ...
    -lm -lpthread -o tools/crash_test
```
