...
- Run `scripts/test_license_sdk_lib.py` with `SYNRIX_LIB_PATH` set to the folder containing `libsynrix.dll` (e.g. `build/windows/build_msys2/bin`). Optionally run `build/windows/tools/test_release_zip.py` after creating the zip.

## 2. Windows release

- Create `synrix-windows.zip`: DLL, OpenSSL/runtime DLLs, optional README.
...
- Build: `build/linux/build.sh` (lattice + exact_name_index + license_utils + synrix_license_linux).
- Attach tarball to the same GitHub release.
- SHA256 for each asset is shown on the release page (no need to paste manually unless desired).
