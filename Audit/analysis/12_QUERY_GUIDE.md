...
```
Good prefix scheme (also accepted by the engine):
...
Bad prefix scheme — these will be REJECTED by the engine:
  preference_dark_mode        ← Not a valid semantic prefix; add_node() will fail
  agent_123_conversation_5    ← Delimiter ambiguity; likely rejected
  my data about python        ← Not prefix-searchable AND will be rejected
```

> **Note**: Synrix enforces a prefix constraint system in `lattice_constraints.c`. Node names must start with a recognized semantic prefix (`LEARNING_`, `ISA_`, `PATTERN_`, `FUNC_`, `DOMAIN_`, `CONSTRAINT_`, `FAILURE_`, `TASK_`, or namespace prefixes like `AGENT_*:`, `USER_*:`, `SESSION_*:`). Names that don't match these patterns are **rejected with an error** — not just suboptimal. Design your naming scheme around the allowed prefixes.
