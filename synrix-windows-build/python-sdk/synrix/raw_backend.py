...
# Load license key from ~/.synrix/license_key if SYNRIX_LICENSE_KEY not set (one-time save, then automatic)
try:
    from .license_key_storage import ensure_license_key_loaded
    ensure_license_key_loaded()
except Exception:
    pass
...
def _find_synrix_lib():
    ...
    if platform.system() == "Windows":
        lib_ext = ".dll"
        # Try free tier DLL first, then standard
    ...
    for lib_name in lib_names:
        ...
        # 2. Package directory first (where Copy-Item puts the DLL: python-sdk/synrix/ or packages/.../synrix/)
        current_file = os.path.abspath(__file__)
        package_dir = os.path.dirname(current_file)  # .../python-sdk/synrix or .../packages/unlimited/synrix
        search_paths.append(os.path.join(package_dir, lib_name))
        
        # 3. Auto-detect relative to project root
        python_sdk_dir = os.path.dirname(package_dir)  # python-sdk/ or packages/unlimited/
        project_root = os.path.dirname(python_sdk_dir)  # project root (may be synrix-windows-build or parent)
        ...
        # Check Windows build location (MSYS2 and legacy paths)
        if platform.system() == "Windows":
            build_paths = [
                os.path.join(project_root, "build", "windows", "build_msys2", "bin", lib_name),  # MSYS2 build (clean rebuild output)
